...
function ready() { /*Это в общем функция, которая подключае все наши кнопки. И чтобы она начала работать ТОЛЬКО после загрузки всего документа, мы пишем цикл (см.Выше)*/
    var removeCartItemButtons = document.getElementsByClassName('btn-danger') /*Переменная теперь хранит все кнопки удаления товара*/
    for (var i = 0; i < removeCartItemButtons.length; i++) { /*цикл нужен для того, чтобы мы могли проходить по всем таким кнопкам */
        var button = removeCartItemButtons[i] /*Эта переменная теперь хранит ту кнопку, которую мы жмакнули*/
        button.addEventListener('click', removeCartItem) /*по клику выполняется функция, которая удаляет итем */
    }

    var quantityInputs = document.getElementsByClassName('cart-quantity-input') /*поле количества*/
    ...
    var addToCartButtons = document.getElementsByClassName('shop-item-button') /*Кнопка добавления в корзину*/
...
function removeCartItem(event) {
    var buttonClicked = event.target /*Определяем ту кнопку, на которую кликнули*/
...
function quantityChanged(event) {/*Проверяем, является ли число в поле ввода количества вообще числом, и положительное ли оно*/
    var input = event.target /*Ловим то, че ввел пользователь*/
...
function addToCartClicked(event) { /*Функция принимает событие (кликнуто)*/
    var button = event.target /*Ищем кнопку,куда тыкнули*/
    var shopItem = button.parentElement.parentElement /*Ищем родителя родителя кнопки, чтобы оттуда забрать наименование товара*/
    var title = shopItem.getElementsByClassName('shop-item-title')[0].innerText /*Назначем переменную с текстом из наименования товара*/
    var price = shopItem.getElementsByClassName('shop-item-price')[0].innerText /*Назначаем переменную, которая забирает текст с ценой товара*/
    var imageSrc = shopItem.getElementsByClassName('shop-item-image')[0].src /*Тоже самое с картинкой*/
...
function addItemToCart(title, price, imageSrc) { /*Функция берет переменные, которые мы ей передали выше*/
    var cartRow = document.createElement('div')/*Создаем строку (точнее блок)*/
    cartRow.classList.add('cart-row') /*Добавляем стили для нашей новой  сгенерированной корзины*/
    var cartItems = document.getElementsByClassName('cart-items')[0] /*Теперь в html документе мы ищем блок cart-items, который предусмотрительно сделали и которому задали стили*/
    var cartItemNames = cartItems.getElementsByClassName('cart-item-title') /*Теперь ищем наименование товара,который мы добавили в корозину*/
    for (var i = 0; i < cartItemNames.length; i++) { /*И проверяем, если такой товар есть в корзине то мы выводим сообщение, что товар уже добавлен*/
...
function updateCartTotal() { /*Эта функция будет изменять цену при изменении количества*/
    var cartItemContainer = document.getElementsByClassName('cart-items')[0] /*Сперва находим родителя строки корзины.Индекс говорит о том, что мы хотим взять самый первый элемент массива карт-итемс(Это вообще вся корзина)*/
    var cartRows = cartItemContainer.getElementsByClassName('cart-row') /*теперь находим строку корзины*/
    var total = 0 /*Изначально ставим цену ноль, в корзинке пусто*/
    for (var i = 0; i < cartRows.length; i++) { /*Затем паеребираем массив со строками корзины */
        var cartRow = cartRows[i] /*Задаем переменную, устанавливаем ей значение, равное любому элементу массива со строками*/
        var priceElement = cartRow.getElementsByClassName('cart-price')[0] /*Создаем переменную для получения цены товара и нам снова нужен самый первый элемент*/
        var quantityElement = cartRow.getElementsByClassName('cart-quantity-input')[0] //Создаем переменную для получения количества товаров*/
        var price = parseFloat(priceElement.innerText) /*До этого момента мы имели сам элемент, теперь же мы получаем текстовое значние. И чтобы это значение было не строкой, а числом мы используем parseFloat*/
        var quantity = quantityElement.value/*Теперь нужно получить значение инпута с количествоим, поэтому используем value*/
...
function round (e){
    var butWrap = $(this).parents('.shop-item-details');
